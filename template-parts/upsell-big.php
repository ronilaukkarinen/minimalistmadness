    ...
    <div class="post-feed">
      ...
      <article class="entry post-card post no-animation">
        <div class="post-card-content">
          ...
          <div class="post-card-image"><div class="img"><p class="post-card-details"><time datetime="<?php the_time( 'c' ); ?>"><?php the_time( 'j.' ); ?> <?php the_time( 'F' ); ?>ta <?php the_time( 'Y' ); ?></time><br /><?php echo khonsu_estimated_reading_time(); // phpcs:ignore ?></p><div class="image image-background image-background-layer"><?php if ( has_post_thumbnail($post->ID) ) { native_lazyload_tag( get_post_thumbnail_id( $post->ID, 'large' ), [ 'sizes' => [ 'big' => 'large' ] ] ); } ?></div></div></div>
